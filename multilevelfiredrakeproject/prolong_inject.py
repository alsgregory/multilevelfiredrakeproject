...
# Make sure that we don't explicitely take as arguments the levels of
# these functions as we can just work these out from get_levels. Remember
# warnings about is they are part of hierarchy or not!!!! -> test this!
...
def ProlongUpToFinestLevel(Function, Hierarchy):
    ...
    if CoarseIndex == -1:
        raise IndexError(
            'Coarse Index is not an index one can prolong from. Function is not in a hierarchy')
...
def InjectDownToAnyLevel(Level_to_inject_to, Function, Hierarchy):
    ...
    if FineIndex == -1:
        raise IndexError(
            'Fine Index is not an index one can inject from. Function is not in a hierarchy')
