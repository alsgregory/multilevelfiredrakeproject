...
class EnsembleHierarchy(object):
    ...
    def __init__(self, M=2):
        ...
        # : This is either 'Data' or 'Function' depending on what the entries into :attr:`EnsembleHierarchy.Ensemble` are.
        ...
        # : Either 'Data' or 'Function' of the MLMC estimator of the expectation of the ensembles.
        ...
        # : Same size as :attr:`EnsembleHierarchy.Ensemble` giving weights of the indidivual ensemble members. Default is evenly weighted.
        self.Weights = []
            ...
            for j in range(len(self.Ensemble[i])):
                for k in range(len(self.Ensemble[i][j])):
                    if len(self.Ensemble[i][j][k].dat.data) != Comparison:
                        raise ValueError(
                            'All Ensemble Members Do Not Have Same Function Spaces')
    ...
    def AppendToEnsemble(self, tuple_to_append, Level_to_append_to):
        ...
        if not isinstance(tuple_to_append, tuple):
        ...
        if self.L + 1 <= Level_to_append_to:
            raise ValueError(
                'Cant append to level more than 1 above EnsembleHierarchy.L')
        
        if Level_to_append_to < len(
                self.Ensemble):  # if there already exists that level in ensemble hierarchy
            
            if not isinstance(tuple_to_append[0], Function):
            ...
            self.Weights[Level_to_append_to] = tuple([np.ones(len(self.Ensemble[Level_to_append_to])) * (1 / float(len(self.Ensemble[
                                                     Level_to_append_to]))), np.ones(len(self.Ensemble[Level_to_append_to])) * (1 / float(len(self.Ensemble[Level_to_append_to])))])
        
        if Level_to_append_to >= len(
                self.Ensemble):  # if the level doesn't exist in ensemble hierarchy
            ...
            self.nxl.append((tuple_to_append[1].function_space().mesh().topology.num_cells() / 2.0) * (self.M**(
                tuple_to_append[1].function_space().mesh().geometric_dimension() * (-lvl + Level_to_append_to))))
            
            self.__OriginalFunctionSpaces.append(tuple(
                [tuple_to_append[0].function_space(), tuple_to_append[1].function_space()]))
            self.__SameFunctionSpaceErrorCheck()  # check for all same function space
            self.Weights[Level_to_append_to] = tuple([np.ones(len(self.Ensemble[Level_to_append_to])) * (1 / float(len(self.Ensemble[
                                                     Level_to_append_to]))), np.ones(len(self.Ensemble[Level_to_append_to])) * (1 / float(len(self.Ensemble[Level_to_append_to])))])
        ...
        if self.Type != To:  # only change type if not already that
            if To == "Data":
                ...
                for i in range(len(self.Ensemble)):
                    ...
                    R = list(np.shape(zerofunction.dat.data))
                    R.append(N)
...
class CopyEnsembleHierarchy(object):
